...
export function FlagsPanel({ flags }: { flags: Flag[] }) {
  if (flags.length === 0) {
    return <p className="text-sm text-neutral-500">No flags — nothing crossed a threshold in this range.</p>;
  }
  return (
    <ul className="flex flex-col gap-2">
      {flags.map((f, i) => (
        <li
          key={`${f.type}-${i}`}
          className={`flex items-start gap-2 rounded-md border px-3 py-2 text-sm ${SEVERITY_CLASSES[f.severity]}`}
        >
